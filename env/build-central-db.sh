...
> ./customizations/"$PCF_ENV"-director$(date +%Y-%m-%d).yml \
...
echo -e "  ./env/bosh/company-ops.yml                      "
echo -e "======================================================"
echo -e "$(tput sgr0)"
# Create Director
...
credhub set -n /ldap_user -t user -z provCOReadOnly@xyz.com -w 'QPv!81oCKLWVf'
...
# Need to manually set default ca and certs
credhub set -n /certs/default_ca --type certificate -r customizations/certs/"$PCF_ENV"/default_ca.ca -c customizations/certs/"$PCF_ENV"/default_ca.certificate -p customizations/certs/"$PCF_ENV"/default_ca.private_key
credhub set -n /certs/wildcard --type certificate -r customizations/certs/"$PCF_ENV"/default_ca.ca -c customizations/certs/"$PCF_ENV"/wildcard.certificate -p customizations/certs/"$PCF_ENV"/wildcard.private_key
credhub set -n /certs/director --type certificate -r customizations/certs/"$PCF_ENV"/director.ca -c customizations/certs/"$PCF_ENV"/director.certificate -p customizations/certs/"$PCF_ENV"/director.private_key
...
wget https://bosh.io/d/github.com/cloudfoundry/syslog-release?v=11.3.2 -O /tmp/syslog_runtime.tgz --no-check-certificate && \
bosh -e $PCF_ENV upload-release /tmp/syslog_runtime.tgz && \
rm /tmp/syslog_runtime.tgz && bosh -e $PCF_ENV update-runtime-config runtime-configs/syslog.yml --name syslog -l ./customizations/"$PCF_ENV"-params.yml -n && \
...
wget https://storage.googleapis.com/harbor-bosh-releases/harbor-container-registry-1.6.2.tgz -O /tmp/harbor.tgz --no-check-certificate && \
bosh -e $PCF_ENV upload-release /tmp/harbor.tgz && \
rm /tmp/harbor.tgz && bosh -e $PCF_ENV update-runtime-config harbor-boshrelease/manifests/harbor.yml --name harbor -n && \
...
echo -e "$(tput rev)"
echo -e "======================"
echo -e "Deploying Harbor      "
echo -e "======================"
echo -e "$(tput sgr0)"
bosh -e $PCF_ENV -d harbor-deployment deploy ./env/harbor/harbor.yml \
