...
rm /tmp/syslog_runtime.tgz && bosh -e $PCF_ENV update-runtime-config runtime-configs/syslog.yml --name syslog -n && \
...
rm /tmp/garden.tgz && \
wget https://bosh.io/d/github.com/cloudfoundry/syslog-release?v="$SYSLOG_RELEASE" -O /tmp/syslog.tgz --no-check-certificate && \
bosh -e $PCF_ENV upload-release /tmp/syslog.tgz && \
rm /tmp/syslog.tgz && \
